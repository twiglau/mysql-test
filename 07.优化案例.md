...
- 列出 class 表里的所有记录。对于每一条 class 记录，如果它对应的 card 号码在 book 表里有对应的书，或者在 phone 表里有对应的手机，就会把相关信息显示在一行里；如果没有对应的，就留空。
...
-- 结论：2个表的type 变为 ref，且总 rows 优化很好。
-- > 说明了，索引最好设置在需要经常查询的字段上。

-- Join语句的优化：
-- 1. 尽可能减少 Join 语句中的NestedLoop的循环总次数；“永远用小结果集 驱动 大结果集”
-- 2. 优先优化NestedLoop的内层循环；
-- 3. 保证Join语句中被驱动表上Join条件字段已经被索引覆盖。
-- 4. 当无法保证被驱动表的Join条件字段被索引覆盖,且内存资源充足的前提下，不要太吝啬 JoinBuffer的设置；
```
